...
- `flutter analyze` 클린 / `flutter test` **14개 통과**
...
| 등락률 Top-20 라이브 | `SplayTreeSet`으로 정렬 유지, 변동 종목만 재삽입 | `_ranked` |
...
측정 원본은 `perf/runs/run_1..5.json`에 보존했다.
...
| 문서 | 내용 |
|---|---|
| **[DESIGN.md](DESIGN.md)** | 설계 판단과 그 근거. 기각한 대안, 구현 여정(목록 최적화 → 초성 검색) |
| **[PERF.md](PERF.md)** | 측정 방법론(왜 `pump()`인가), 5회 중앙값 수치, 해석, **효과 없어 기각한 가설** |
...
- **99th/worst 꼬리 +26~29%** — 원인은 랭킹 버스트(Top-20 재계산 + 트리 갱신이 한 프레임에 몰림).
  다음 방향은 "프레임당 작업량 상한 + 초과분 이연"이며 아직 **측정으로 검증하지 않은 가설**이다.
...
- old-gen GC +71%의 원인을 측정으로 분리하지 못했다(promotion 추정에 머묾).
